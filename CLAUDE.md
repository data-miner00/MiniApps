...
MiniApps is a collection of small, standalone desktop/console apps, each in its own project folder under `src/` and added to `MiniApps.sln`. Per the README:
...
- When adding a new app, create a new project folder under `src/` + `.csproj` and add it to `MiniApps.sln`, following the existing pattern (see `ODataQuerySimulator` for a console app, `Borderless` for a WPF app).
...
dotnet run --project src/ODataQuerySimulator     # run the OData console app
dotnet run --project src/CronExpressionSimulator # run the cron expression console app
dotnet run --project src/RegexTester             # run the regex tester console app
dotnet run --project src/Borderless              # run the borderless WPF app
dotnet run --project src/ScreenRuler             # run the screen ruler WPF app
```
...
- **Borderless** (`src/Borderless/`) — WPF app (`net10.0-windows`), `OutputType=WinExe`. A single borderless, non-resizable window (`WindowStyle="None"`, `ResizeMode="NoResize"`) that displays an image (`Image/dawn.jpg`) and is dragged by left-click-and-drag on the window body (`Window_MouseDown` → `DragMove()`). Minimal single-window/single-code-behind structure (`MainWindow.xaml` + `MainWindow.xaml.cs`), no MVVM/DI.
- **ODataQuerySimulator** (`src/ODataQuerySimulator/`) — Console app (`net10.0`) built on `Microsoft.OData.Core`/`Microsoft.OData.Edm`. Everything lives in `Program.cs`:
...
- **CronExpressionSimulator** (`src/CronExpressionSimulator/`) — Console app (`net10.0`) built on the [Cronos](https://github.com/HangfireIO/Cronos) NuGet package for cron parsing/scheduling. Everything lives in `Program.cs`: a REPL (`cron>` prompt) that parses a cron expression via `CronExpression.Parse` and prints the next 10 occurrences. Supports standard 5-field expressions, an optional leading seconds field (auto-detected by field count in `ParseFormat`), and `@` macros (`@daily`, `@hourly`, etc.). `GetNextOccurrence` requires the `from` timestamp to be UTC when a `TimeZoneInfo` is supplied — internal timestamps are kept in UTC and only converted to local time for display. Supports `help`/`exit`/`quit` commands.
- **RegexTester** (`src/RegexTester/`) — Console app (`net10.0`), no external packages (uses `System.Text.RegularExpressions` from the BCL). Everything lives in `Program.cs`: a two-mode REPL. `pattern>` prompt compiles a `.NET` regex (a 2s `MatchTimeout` guards against catastrophic backtracking); on success it switches to `test [pattern]>` prompt, where each line is matched against the compiled pattern and printed with match/group indices. Inline options (`(?i)`, `(?m)`, etc.) are the supported way to set flags rather than a custom flags syntax. Supports `help`, `pattern` (return to pattern mode), `exit`/`quit` commands.
- **ScreenRuler** (`src/ScreenRuler/`) — WPF app (`net10.0-windows`), `OutputType=WinExe`. `MainWindow` is a small control panel (radio buttons + Start button) that launches one of two on-screen measuring tools, both always-on-top and closable via Esc: `MeasureOverlayWindow` is a fullscreen transparent overlay (same pattern as ColorPicker's `PickerOverlayWindow`) where click-dragging draws a line and shows live distance (px) and angle (°) between the two points; `RulerBarWindow` is a semi-transparent, resizable bar with procedurally-drawn tick marks (redrawn on `SizeChanged`) that you resize via `WindowChrome`-provided edge hit-testing and move by dragging its body (`DragMove()`, as in `Borderless`), with a button to toggle horizontal/vertical orientation. All measurements are in WPF logical pixels (DIPs), not device pixels — no DPI-awareness handling, consistent with `ColorPicker`'s existing scope.
